...
// استيراد المكونات المخصصة والآيقونات
import { AddMedicineButton } from "../components/AddMedicineButton";
import LayersOutlinedIcon from "@mui/icons-material/LayersOutlined";
import CheckCircleOutlinedIcon from "@mui/icons-material/CheckCircleOutlined";
import EventBusyOutlinedIcon from "@mui/icons-material/EventBusyOutlined";
...
import { useGetData } from "../../../shared/hooks/useGetData";
import type { FilterStatus, RawBatchData } from "../types/batches";
import { AddOpeningStockDialog } from "../components/AddBatchDialog";

// ==========================================
// Custom Hook (الخطاف المخصص للمنطق)
// ==========================================
export const useMedicineBatches = (rawBatches: RawBatchData[]) => {
  const [activeFilter, setActiveFilter] = useState<FilterStatus>("ALL");
...
// ==========================================
// Main Component (مكون الواجهة الرئيسي)
// ==========================================
export const DrugBatchesPage: React.FC = () => {
  const { drugId } = useParams<{ drugId: string }>();
  const location = useLocation();
  ...
  const drugName = stateData?.drugName || "دواء غير محدد";
  const activeIngredient = stateData?.activeIngredient || "المادة الفعالة";
  ...
  return (
    ...
    >
      ...
      {/* نافذة إضافة بضاعة أول المدة */}
      <AddOpeningStockDialog
        open={isAddBatchOpen}
        onClose={() => setIsAddBatchOpen(false)}
        drugName={drugName}
        pharmacyDrugId={drugId ? Number(drugId) : null}
      />
    </Box>
  );
};
