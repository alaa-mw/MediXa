...
//   return (
//     <CommandCenter>
//       {/*الأزرار الخاصة بالبحث للتبديل بين السيرش*/}
...
//                   if (
//                     searchTerm.trim() !== "" ||
//                     (mode === "INGREDIENT" && hasSelectedIngredients)
//                   ) {
//                     setIsDropdownOpen(true);
//                   }
//                 }}
...
//                       setSearchTerm("");
//                     }}
//                     onSelectionChange={(hasItems) =>
//                       setHasSelectedIngredients(hasItems)
//                     }
//                   />
//                 )}
//               </Paper>
...
// استيراد مكون الباركود والـ Hook الخاص بالفاتورة
import BarcodeMyDrugs from "../../../../shared/layout/BarcodeMyDrugs"; // عدّل المسار حسب هيكلة مجلداتك
...
export const DrugSearchArea = () => {
  ...
  // معالجة النتيجة المجلوبة عند مسح الباركود
  const handleBarcodeScanResult = (result: { id: string; tradeName: string }) => {
    const drugId = Number(result.id);
    if (drugId) {
      actions.addDrug(drugId, {
  ...
  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value.trim() !== "") {
      ...
      // لا تغلق القائمة إذا كنا في وضع المادة الفعالة وهناك مواد مختارة مسبقاً
      if (mode === "INGREDIENT" && hasSelectedIngredients) {
        setIsDropdownOpen(true);
      } else {
        setIsDropdownOpen(false);
  ...
  return (
    <CommandCenter>
      ...
      <Box sx={{ display: "flex", gap: "12px", alignItems: "center" }}>
        ...
        {/* زر ومكون قارئ الباركود */}
        <Tooltip title="قارئ الباركود نشط">
          <ActionButton>
            <BarcodeMyDrugs onFindResult={handleBarcodeScanResult} />
          </ActionButton>
        </Tooltip>
      </Box>
    </CommandCenter>
